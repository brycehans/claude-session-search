...
# Fetch full transcript for a known session ID
./claude_session_search.py --session <session-id>
./claude_session_search.py --session <prefix> --project /path/to/project
```
...
**Data pipeline (search mode):** `resolve_project_dir` → `load_sessions` → `filter_sessions` → `extract_messages` → `search_messages` → `format_*_output`

**Data pipeline (session mode):** `find_session_file` → `extract_messages` → `format_transcript` / JSON output
...
- **find_session_file**: Locates a session JSONL file by full or prefix ID across project directories
- **format_transcript**: Formats a full session transcript for terminal display (used by `--session`)
